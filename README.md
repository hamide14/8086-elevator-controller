...
elevator_layer8_swint.asm    Main 8086 assembly source (heavily commented)
elevator_layer8_swint_even.bin   Even-byte ROM image
elevator_layer8_swint_odd.bin    Odd-byte ROM image
...
3. To modify the assembly and rebuild: assemble `elevator_layer8_swint.asm`, then split the resulting binary into even/odd byte banks to match the 16-bit-bus / 8-bit-memory-chip layout before loading it back into Proteus.
